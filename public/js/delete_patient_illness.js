...
let deletePatientIllnessForm = document.getElementById(
...
deletePatientIllnessForm.addEventListener("submit", function (e) {
  ...
  xhttp.open("DELETE", "/delete-patient-illness", true);
  xhttp.setRequestHeader("Content-type", "application/json");

  // Tell our AJAX request how to resolve
  xhttp.onreadystatechange = () => {
    if (xhttp.readyState == 4 && xhttp.status == 204) {
      deleteRow(inputPatientIllnessId);
    }
  };
...
// Based on:
// Source URL: https://github.com/osu-cs340-ecampus/nodejs-starter-app/tree/main/Step%207%20-%20Dynamically%20Deleting%20Data
function deleteRow(patientIllnessId) {
  // delete row from table and hide the delete section
  let table = document.getElementById("all-patients-illnesses");
  for (let i = 0, row; (row = table.rows[i]); i++) {
    if (table.rows[i].getAttribute("data-value") === patientIllnessId) {
      table.deleteRow(i);
      hideDeleteSection();
      break;
    }
